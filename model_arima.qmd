...
run_arima <- function(estimation_data, fname_suffix = "") {
  ...
  save_arima_results(m_table, get_model_cols(m_table, key_vars(estimation_data)),glue("arima_debth{fname_suffix}_results.csv"))
...
plot_prediction <- function(Bank, Partner, bank_fcasts, all_data) {
    fcast_data <- bank_fcasts |> filter(BankName == Bank, Partner == Partner) |> head(8) 
    ...
    event_data <- all_data |> filter(BankName == Bank, !!as.name(Partner) >=0) |> head(8) 
    ...
    labs(title = Bank, subtitle = glue("{Partner}"), y = glue("{target_label}"))    
}
...
result <- map2(partner_banks$Bank, partner_banks$Partner, get_prediction, data, best_bank_models) |>
            list_rbind() |> left_join(data |> select(observed = UBPRE524.Value, Partner),by = join_by(IDRSSD, BankName, BankType, Quarter) ) |> as_tibble() |> select(Partner,BankName, Quarter, observed, predicted) |> mutate(AR = abs(observed) -abs(predicted))

map2(partner_banks$Bank, partner_banks$Partner, plot_prediction, result, data) 

result |> group_by(Partner,BankName) |> mutate(Period = row_number()) |> summarise(mean_ar = mean(AR, na.rm=TRUE))


result |> group_by(Partner,BankName) |> mutate(Period = row_number()) |> mutate(cum_mean = cummean(AR))

result |> group_by(Partner,BankName) |> mutate(Period = row_number()) |> mutate(cum_sum = cumsum(AR)) 


result |> group_by(Partner,BankName) |> mutate(Period = row_number()) |> 
filter(Period==2) |> summarise(mean_ar = mean(AR, na.rm=TRUE))
