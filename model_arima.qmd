...
get_prediction <- function(bankname, partner, all_data, models) {
  tryCatch({
    event_qtr <- all_data |>
    ...
    original_scale(forecasts, all_data) |> add_column(Partner = partner) |> filter(Quarter >= event_qtr & Quarter < event_qtr + 7)    
...
result <- map2(partner_banks$Bank, partner_banks$Partner, get_prediction, data, best_bank_models) |>
            list_rbind() |> left_join(data |> dplyr::select(observed = UBPRE524.Value),by = join_by(IDRSSD, BankName, BankType, Quarter) ) |> as_tibble() |> dplyr::select(Partner,BankName, Quarter, observed, predicted, .model) |> mutate(AR = abs(observed) -abs(predicted))
