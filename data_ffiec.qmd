...
Of note, US regulators have access to FR Y-14 [@Fed14M] reporting that breaks down the credit card portfolio by additional attributes. However, this is commercially sensitive and not shared [@CFPB2023]. 
...
get_ubpr_labels() |>
filter(str_detect(Desc, "Credit|90 days")) |> 
...
To measure quality, we can use 'Credit Card Plans-30-89 DAYS P/D %' (UBPRE524), which is the proportion of credit cards that are delinquent in the credit card portfolio after 30 days. In studies we see alternative measures of quality include delinquency at 90+ days (UBPRE521), and charge-off rates ([@FedCharge] and [@CFPB2023]).
...
@tbl-peergrpcnt details 3 peer groups. National (US-wide) Credit card partnerships occur between banks in the **Large Bank** and **Large Credit Card Bank** groups. These peer groups are defined by FFIEC (see @sec-peers for full list of Banks in each peer group) [@ffiec]. Banks belong to either group i.e., membership is mutually exclusive. Notably, partnerships do occur for other banks [@TCCP2022], but we will focus on larger partnerships that occur between Large Banks and Large Credit Card banks. 
...
## Data Period {#sec-cardact}

Regulation implemented in 2010 had a material impact on the credit card market [@cfpb2013; also @boston2014]. As highlighted by @boston2014, regulation coincided with the 2008 financial crisis and post recovery period, making it difficult to fully assert causation [-@boston2014].
...
We can see the impact in @fig-regulation which plots a linear trend for *`r target_label`* for each peer group. Based on @fig-regulation-3 there was increasing delinquency for Large Banks and Large Credit Card Banks before regulation, but post regulation the trend has reversed and is now trending down. It does appear that it took banks about a year for the regulatory changes to have an impact. If we take out staring point as 2011 Q3 as in @fig-regulation-4, then the deliquency trend for Large Credit Card Banks is increasing.
...
#|   - "Linear trend pre and post 2011 Q3. About a year after CARD Act part 2 was enacted."
#| layout-ncol: 1
...
Following the methodology proposed by @Hyndman2021, we can use Principal Components Analysis (PCA) across different statistical features or feature sets, to identify bank that diverge from its peers. 
...
7. @fig-lfeature-7 *Large Banks* and *Large Credit Card Banks* is not stationary i.e. the statistical properties of the time series changes, which makes it difficult to identify trends and patterns. Performing a difference i.e. to get the quarterly change in UBPRE524, makes the series stationary as shown in @fig-acf-2. This can be confirmed by performing an augmented Dickey–Fuller (ADF) test on the differenced value. We also see that, post differencing, there are autocorrelation coefficients above the significance threshold. The decreasing magnitude of these coefficients tell us the earlier lags have a stronger influence than values further in the past.
...
#| error: false
#| label: fig-acf
#| fig-cap: "ACF and PACF"
#| fig-subcap:
#|   - "ACF on original value"
#|   - "ACF on first difference value"
#|   - "PACF on original value"
#|   - "PACF on first difference value"
#| layout-ncol: 2
agg_df <- final_df |> 
            group_by(BankType)|> 
...
3. Date Period: from 2010 Q2 or from 2010 Q3 year which is when we see more of a steady state post regulatory change.
