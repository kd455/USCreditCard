...
overdue30to89.group <- credit_card.overdue30to89.agg.group(TRUE,TRUE)

loan_amt_pct_change <- credit_card.loan_amount(TRUE,TRUE) |> group_by(BankName) |> mutate(pct_change = value_diff/lag(Value) *100) |>as_tibble() |>select(Quarter,BankName,loan_amt.pct_change = pct_change)
loan_unused_pct_change <- credit_card.unused(TRUE,TRUE) |> group_by(BankName) |> mutate(pct_change = value_diff/lag(Value) *100) |> as_tibble() |>select(Quarter,BankName,loan_unused.pct_change = pct_change)


event_duration <- 5 # 2 quarters - current + 1
est_duration <- 8
partner_data <- credit_card.partnerships() |> 
                    ...
                    left_join(loan_amt_pct_change) |>
                    left_join(loan_unused_pct_change) |>
                    mutate( Period = Quarter - yearquarter(acquired),
                            EventPeriod = (Period >= 0 & Period < event_duration),
                            EstimationPeriod = (Period < 0 & Period >= -est_duration),
                            Qtr = quarter(Quarter)) |> 
                    left_join(overdue30to89.group, by = join_by(BankType, Quarter)) |>
                    drop_na(value_diff) |> rename(partner_name = name)
...
estimation_data <- partner_data |> 
                    filter(EstimationPeriod == TRUE) |> 
                    add_row(overdue30to89 |> filter(!BankName %in% partner_data$BankName) |>
                                left_join(loan_amt_pct_change, by = join_by(BankName, Quarter)) |>
                                left_join(loan_unused_pct_change, by = join_by(BankName, Quarter)) |>
                                left_join(overdue30to89.group, by = join_by(BankType, Quarter)))|>
                    mutate( #value_diff.detrend = value_diff - value_diff.group.mean,
                            #partner_event = !is.na(name),
                            EstimationPeriod = TRUE,
                            Qtr = quarter(Quarter)) |>
                            #Year = year(Quarter)) |> 
                    drop_na(value_diff) 

bank_mean <- estimation_data |> 
                group_by(BankName) |> summarise(value_diff.est.mean = mean(value_diff,na.rm=TRUE))

results <- tibble(model = character(),
                    rsme = numeric())

market_formula <- "value_diff ~ value_diff.group.mean"
fixed_formula <- "value_diff ~ Qtr + factor(BankType)"
mixed_formula <- "value_diff ~ Qtr * BankType + (1 | IDRSSD)"
hier_formula <- "value_diff ~ Qtr * BankType + (1 | BankType/IDRSSD) + loan_amt.pct_change"
# Fit the models
m_market <- lm(as.formula(market_formula), data = estimation_data)
m_fixed <- lm(as.formula(fixed_formula), data = estimation_data)
