...
get_summary_validation <- function(model_name, banks) {
  ...
  list(tbl1 = summary_banks, tbl2 = summary_mean_banks, tbl3 = summary_mean_all)
}
...
![Parameter Tuning for model with group predictors](images/xgbTree_tuning_plot_model_w_group.png){#fig-xgcv-group}
![Parameter Tuning for model without group predictors](images/xgbTree_tuning_plot_model_w_group.png){#fig-xgcv-wo-group}
...
#| tbl-cap: Tuning model parameters 
...
bind_rows(params_w_group, params_wo_group) |> 
...
importance_matrix <- read_csv("data/results/xgbTree_importmatrix_w_group.csv",show_col_types = FALSE)

feature_desc <- get_feature_labels()

importance_matrix_lab <- importance_matrix |> 
...
xgb.plot.importance(importance_matrix_lab, rel_to_first = TRUE, xlab = "Relative importance", top_n = 12,left_margin = 30)

```
## Feature Importance by Frequency

```{r}
xgb.ggplot.importance(importance_matrix_lab, measure = "Frequency", rel_to_first = TRUE,top_n = 12,left_margin = 30) + ylab("Freqency")
```
...
```{r}
#| warning: false
selected_features <- importance_matrix |> 
                      dplyr::select(Feature) |> 
                        filter(!grepl("BankName", Feature)) |> top_n(15) |> as.list()
...
```
![Parameter Tuning for model with selected predictors](images/xgbTree_tuning_plot_model_selected.png){#fig-xgcv-selected}
...
```{r}
banks = unique(observation_data$BankName)
val_results <- get_summary_validation("model_select", banks)
val_results$tbl1 |> rmarkdown::paged_table()
val_results$tbl2 |> rmarkdown::paged_table()
val_results$tbl3 |> rmarkdown::paged_table()
```
...
Feature Importance: SHAP values per feature for training and test dataset.
:::
...
Waterfall: mean SHAP contribution of features for training and test dataset.
:::
