...
data <- model_data$all_data |> as_tibble()           
partner_banks <- credit_card.partnerships.long()

log_change_cols <- data |>
                    dplyr::select(contains(".log.diff")) |> 
                    dplyr::select(-c(starts_with("UBPRE524.log.diff"))) |> names() 
                               # starts_with("UBPRE524.group"), 
                               # starts_with("UBPRE524.all"))) 

est_data_wQuarter <- model_data$estimation_data |> as_tibble() |>
                      dplyr::select(c(Quarter, UBPRE524.Value,UBPRE524.diff, BankName, BankType, all_of(log_change_cols), Qtr))

estimation_data <- est_data_wQuarter |> dplyr::select(-c(Quarter, UBPRE524.Value)) 

obs_data_wQuarter <- model_data$observation_data |> as_tibble()

observation_data <- obs_data_wQuarter|> 
                      dplyr::select(c(UBPRE524.diff, BankName, BankType, all_of(log_change_cols), Qtr))
...
comb |> 
  filter(BankName %in% observation_data$BankName) |> 
    arrange(BankName,.type) |> mutate(across(where(is.numeric), \(x) round(x,3))) |> rmarkdown::paged_table()     
...
obs_data_wQuarter$prediction <- predictions
...
result <- map2(partner_banks$Bank, partner_banks$Partner,map_prediction_to_partner, obs_data_wQuarter, data) |>
            list_rbind()
