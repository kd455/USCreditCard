...
log_change_cols <- data |>
                    select(contains(".log.diff")) |> 
                    select(-c(starts_with("UBPRE524.log.diff"),
                                starts_with("UBPRE524.group"), 
                                starts_with("UBPRE524.all"))) |> names() 

est_data_wQuarter <- model_data$estimation_data |> as_tibble() |>
                      select(c(Quarter, UBPRE524.Value,UBPRE524.diff, BankName, BankType, all_of(log_change_cols), Qtr))

estimation_data <- est_data_wQuarter |> select(-c(Quarter, UBPRE524.Value)) 

obs_data_wQuarter <- model_data$observation_data |> as_tibble()

observation_data <- obs_data_wQuarter|> 
                      select(c(UBPRE524.diff, BankName, BankType, all_of(log_change_cols), Qtr))
