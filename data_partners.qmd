...
### FICO Score
The *Terms of credit card plans (TCCP) survey* [@TCCP2022] provides credit card details including their target FICO Tiers and Annual Percentage Rate (APR). We can see from @tbl-tccp-apr that the APR is higher for partnership cards. @boston2000 found that banks faced an adverse selection problem whereby it was profitable to offer credit card deals with high APR that attracted risker borrowers even though there was evidence of higher delinquency. 
...
tccp |> group_by(`Institution Name`, Partner) |> summarise(MaxAPR = median(MaxAPR, na.rm=TRUE)) |>
group_by(Partner) |> summarise(MaxAPR = median(MaxAPR, na.rm=TRUE)) |> mutate(Partner = if_else(Partner == 1 , "Yes", "No"), MaxAPR = MaxAPR*100) |> dplyr::select(`Partnership card` = Partner, `Max APR (%)` = MaxAPR)
```
