...
m9 <- lmer(UBPRE524.diff ~ (1 + UBPRB538.log.diff | BankType/BankName) + UBPRE524.all.log.diff + UBPRE524.group.log.diff,data = estimation_data, REML = FALSE)
```
```{r}
#| warning: false
#| label: tbl-hier-aicc
...
### Model Choice
The best model includes aggregated values for `r credit_card.target_label()`. This allows us to control for the impact of the group trends on the individual banks.
...
m8.9 <- lmer(UBPRE524.diff ~ (1 + UBPRB538.log.diff + UBPR3815.log.diff + Qtr | BankName) + UBPRE524.group.log.diff, data = estimation_data, REML = FALSE)
...
Analysis of the correlation matrix @tbl-corr-hier-1 reveals a notable correlation of -0.87	between UBPRE524.group.log.diff and UBPRE524.all.log.diff. This is corroborated by the Variance Inflation Factor (VIF) analysis @tbl-corr-hier-2. VIF quantifies the extent to which the variance of an estimated regression coefficient increases due to multicollinearity among the predictors. A high VIF signifies that a predictor shares a substantial amount of information with other predictors in the model, potentially distorting the regression coefficients and their interpretability. According to @rproject, VIF values above 4 warrant further investigation, while VIFs exceeding 10 are signs of serious multicollinearity requiring correction.
...
From @fig-random-effect we can see that the random effect of *Unused Commitments on Credit Cards* (UBPR3815) in particular varies widely across banks and quarters.
