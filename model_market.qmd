...
plot_prediction <- function(Bank, Partner, bank_fcasts, all_data) {
    ...
    quarter_prior <- all_data |> filter(BankName == Bank, !!as.name(Partner) ==-1)
    event_data <- all_data |> filter(BankName == Bank, !!as.name(Partner) >=0) |> head(8) 
    comb_data <- bind_rows(est_data_trunc,quarter_prior,event_data)
...
result |> group_by(Partner,BankName) |> mutate(Period = row_number()) |> mutate(cum_mean = cummean(AR)) |> View()

result |> group_by(Partner,BankName) |> mutate(Period = row_number()) |> mutate(cum_sum = cumsum(AR)) |> View()
