...
result <- map2(partner_banks$Bank, partner_banks$Partner, get_prediction, data, m_table)  |> 
  list_rbind() |> left_join(data |> select(observed = UBPRE524.Value),by = join_by(IDRSSD, BankName, BankType, Quarter) ) |> as_tibble() |> select(Partner, BankName, Quarter, observed, predicted) 
