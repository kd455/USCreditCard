...
m_table <- estimation_data |> fill_gaps() |> fabletools::model(tslm = TSLM(as.formula(market_formula)))
...
m_table |> filter(BankName %in% observation_data$BankName) |> glance() |> relocate(BankName, AICc, AIC) |> dplyr::select(-IDRSSD)|>
...
cv_results_est <- readr::read_csv("data/results/estimate_market_metrics.csv",show_col_types = FALSE) |> dplyr::select(-.model) |>
  mutate(across(where(is.numeric), \(x) round(x,4))) |> 
  dplyr::select(BankName, .type, RMSE, MAE)

cv_results_test <-readr::read_csv("data/results/estimate_tscv_market_metrics.csv",show_col_types = FALSE) |> dplyr::select(-.model) |>
  mutate(across(where(is.numeric), \(x) round(x,4))) |> 
